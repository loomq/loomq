 ...
 * 测试共享 WheelConfig 工厂。
 *
 * <p>收敛 19 个测试文件里逐字重复的 9 参魔数字面量(30/16/1/10_000/60min/60s),
...
public final class TestWheelConfigs {
    ...
    public static WheelConfig defaults(Path dataDir, int slotsPerBucket) {
        return new WheelConfig(dataDir.toString(), "t", 30, slotsPerBucket, 1, 10_000L,
            60L * 60_000L, 60_000L, null);
    }

    /** 慢组提交变体(awaitCommit 超时/内联 force 兜底相关测试用)。 */
    public static WheelConfig slowCommit(Path dataDir) {
        return new WheelConfig(dataDir.toString(), "t", 30, 16, 60_000L, 50L,
            60L * 60_000L, 60_000L, null);
    }
}
